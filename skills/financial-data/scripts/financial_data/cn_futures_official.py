    ...
    *,
    ...
    if not isinstance(payload, dict) or "o_curinstrument" not in payload or not isinstance(payload["o_curinstrument"], list):
        raise FinancialDataError(
            ...
            f"{exchange} daily payload missing o_curinstrument",
            {"source_url": source_url},
        )
    ...
    for raw in payload["o_curinstrument"]:
        ...
        if not product:
            raise FinancialDataError(
                ErrorCode.NORMALIZATION_ERROR,
                f"{exchange} row missing product code",
                {"row": raw, "source_url": source_url},
            )
        ...
        out.append(
            _canonical_row(
                ...
                open_=raw.get("OPENPRICE"),
                high=raw.get("HIGHESTPRICE"),
                low=raw.get("LOWESTPRICE"),
                close=raw.get("CLOSEPRICE"),
                settlement=raw.get("SETTLEMENTPRICE"),
                pre_settlement=raw.get("PRESETTLEMENTPRICE"),
...
def parse_shfe_daily_payload(payload: Any, trade_date: Any, source_url: str) -> list[dict[str, Any]]:
    return _structured_exchange_rows(
        payload,
        trade_date=trade_date,
        exchange="SHFE",
        source_id="shfe",
...
def _json_rows(payload: Any, exchange: str, source_url: str) -> list[dict[str, Any]]:
    if not isinstance(payload, dict) or not isinstance(payload.get("data"), list):
        raise FinancialDataError(
            ErrorCode.NORMALIZATION_ERROR,
            f"{exchange} daily payload missing data list",
            {"source_url": source_url},
        )
    return payload["data"]
...
def parse_dce_daily_payload(payload: Any, trade_date: Any, source_url: str) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for raw in _json_rows(payload, "DCE", source_url):
        ...
        out.append(
            _canonical_row(
                ...
                low=raw.get("low"),
                close=raw.get("close"),
                settlement=raw.get("clearPrice"),
                pre_settlement=raw.get("lastClear"),
                volume=raw.get("volumn"),
                turnover=raw.get("turnover"),
                open_interest=raw.get("openInterest"),
                source_id="dce",
                source_url=source_url,
                turnover_unit="provider_declared",
                raw=raw,
                delivery_month=_delivery_month(cid),
            )
        )
    return out
...
def parse_gfex_daily_payload(payload: Any, trade_date: Any, source_url: str) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for raw in _json_rows(payload, "GFEX", source_url):
        name = str(raw.get("variety", ""))
        ...
        out.append(
            _canonical_row(
                ...
                turnover_unit="provider_declared",
                raw=raw,
                delivery_month=month or _delivery_month(cid),
            )
        )
    return out
...
def parse_cffex_daily_csv(
    ...
    *,
    ...
    if "合约" not in text or "开盘" not in text:
        raise FinancialDataError(
            ErrorCode.NORMALIZATION_ERROR,
            "CFFEX CSV header not recognized",
            {"source_url": source_url},
        )
    reader = csv.DictReader(io.StringIO(text.lstrip("\ufeff")))
    if not reader.fieldnames:
        raise FinancialDataError(ErrorCode.NORMALIZATION_ERROR, "CFFEX CSV has no header", {"source_url": source_url})
    out: list[dict[str, Any]] = []
    for raw0 in reader:
        raw = {
        ...
        out.append(
            _canonical_row(
                ...
                exchange="CFFEX",
                trade_date=trade_date,
                open_=_first(raw, "今开盘", "开盘价"),
                high=_first(raw, "最高价"),
                low=_first(raw, "最低价"),
                close=_first(raw, "今收盘", "收盘价"),
                settlement=_first(raw, "今结算", "结算价"),
                pre_settlement=_first(raw, "前结算", "前结算价"),
                volume=_first(raw, "成交量"),
                turnover=_first(raw, "成交金额", "成交额"),
                open_interest=_first(raw, "持仓量"),
                source_id="cffex",
                source_url=source_url,
                turnover_unit="CNY_10K",
                raw=raw,
                delivery_month=_delivery_month(cid),
            )
        )
    ...
    *,
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(content)) as zf:
            names = {name.split("/")[-1]: name for name in zf.namelist()}
            if target not in names:
                raise FinancialDataError(
                    ErrorCode.SOURCE_UNAVAILABLE,
                    f"CFFEX archive missing {target}",
                    {"source_url": source_url},
                )
            raw = zf.read(names[target])
    except FinancialDataError:
        raise
    except Exception as exc:
        raise FinancialDataError(
            ErrorCode.NORMALIZATION_ERROR,
            f"invalid CFFEX history ZIP: {exc}",
            {"source_url": source_url},
        ) from exc
    ...
    for encoding in ("gb2312", "gbk", "utf-8-sig"):
        try:
            ...
            continue
    if text is None:
        raise FinancialDataError(ErrorCode.NORMALIZATION_ERROR, "cannot decode CFFEX CSV", {"source_url": source_url})
...
def parse_czce_daily_text(text: str, trade_date: Any, source_url: str) -> list[dict[str, Any]]:
    if "您的访问出错了" in text or ("无期" in text and "行情" in text):
        raise FinancialDataError(
            ErrorCode.SOURCE_UNAVAILABLE,
            "CZCE returned an error/no-data page",
            {"source_url": source_url},
        )
    ...
    if header_idx is None:
        raise FinancialDataError(
            ErrorCode.NORMALIZATION_ERROR,
            "CZCE daily header not recognized",
            {"source_url": source_url},
        )
    ...
    for line in lines[header_idx + 1 :]:
        ...
        out.append(
            _canonical_row(
                ...
                close=_first(raw, "今收盘", "收盘价"),
                settlement=_first(raw, "今结算", "结算价"),
                pre_settlement=_first(raw, "昨结算", "前结算价"),
                volume=_first(raw, "成交量(手)", "成交量"),
                turnover=_first(raw, "成交额(万元)", "成交额"),
                open_interest=_first(raw, "空盘量", "持仓量"),
                source_id="czce",
                source_url=source_url,
                turnover_unit="CNY_10K" if "成交额(万元)" in raw else "provider_declared",
                raw=raw,
                delivery_month=_delivery_month(cid),
            )
        )
...
def fetch_gfex_daily(trade_date: Any, *, client: Optional[HttpClient] = None) -> list[dict[str, Any]]:
    ...
    payload = {"trade_date": compact, "trade_type": "0"}
    headers = {
        "X-Requested-With": "XMLHttpRequest",
        "Referer": "https://www.gfex.com.cn/gfex/rihq/hqsj_tjsj.shtml",
        "User-Agent": "Mozilla/5.0 financial-data/0.2.2",
    }
    return parse_gfex_daily_payload(
        _client(client).post_json(url, data=payload, headers=headers),
        trade_date,
        url,
    )


def fetch_cffex_daily(
    trade_date: Any,
    *,
    ...
    response = _client(client).get_response(
        url,
        headers={"User-Agent": "Mozilla/5.0 financial-data/0.2.2"},
    )
...
def _decode_exchange_text(content: bytes, source_url: str) -> str:
    for encoding in ("utf-8-sig", "gb18030", "gbk"):
        try:
            return content.decode(encoding)
        except UnicodeDecodeError:
            continue
    raise FinancialDataError(ErrorCode.NORMALIZATION_ERROR, "cannot decode exchange text response", {"source_url": source_url})
...
def fetch_czce_daily(trade_date: Any, *, client: Optional[HttpClient] = None) -> list[dict[str, Any]]:
    ...
    if int(year) < 2016:
        raise FinancialDataError(
            ...
            "CZCE READY fetcher supports the modern DFSStaticFiles regime from 2016 onward",
            {"trade_date": normalize_trade_date(trade_date)},
        )
    ...
    return parse_czce_daily_text(_decode_exchange_text(content, url), trade_date, url)


_FETCHERS: dict[str, Callable[..., list[dict[str, Any]]]] = {
    "SHFE": fetch_shfe_daily,
    "INE": fetch_ine_daily,
    "DCE": fetch_dce_daily,
    "CZCE": fetch_czce_daily,
    "CFFEX": fetch_cffex_daily,
    "GFEX": fetch_gfex_daily,
}
