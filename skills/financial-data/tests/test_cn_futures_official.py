...
def test_normalize_and_validation_basics():
    ...
    bad = {
        "contract_id": "CU2609", "trade_date": "2026-08-14",
        "open": 100, "high": 99, "low": 98, "close": 100,
        "volume": -1, "open_interest": 2, "turnover": 3,
    }
...
def test_shfe_structured_payload_filters_summary_and_keeps_settlement():
    payload = {"o_curinstrument": [
        {"PRODUCTGROUPID": "cu", "DELIVERYMONTH": "2609", "OPENPRICE": "80000", "HIGHESTPRICE": "81000", "LOWESTPRICE": "79000", "CLOSEPRICE": "80500", "SETTLEMENTPRICE": "80300", "PRESETTLEMENTPRICE": "79800", "VOLUME": "123", "OPENINTEREST": "789", "TURNOVER": "456.7"},
...
def test_ine_productid_fallback():
    payload = {"o_curinstrument": [{
        "PRODUCTID": "sc_f", "DELIVERYMONTH": "2609", "OPENPRICE": 500, "HIGHESTPRICE": 510, "LOWESTPRICE": 490,
        "CLOSEPRICE": 505, "SETTLEMENTPRICE": 503, "PRESETTLEMENTPRICE": 498, "VOLUME": 3, "OPENINTEREST": 4, "TURNOVER": 5,
    }]}
...
def test_dce_and_gfex_json_rows():
    dce = {"data": [{
        "variety": "豆粕", "contractId": "m2609", "open": "3000", "high": "3100", "low": "2950", "close": "3050",
        "lastClear": "2990", "clearPrice": "3040", "volumn": "100", "openInterest": "200", "turnover": "999",
    }]}
    ...
    gfex = {"data": [{
        "variety": "碳酸锂", "varietyOrder": "lc", "delivMonth": "2609", "open": "80000", "high": "81000", "low": "79000", "close": "80500",
        "lastClear": "79800", "clearPrice": "80300", "volumn": "100", "openInterest": "200", "turnover": "999",
    }]}
...
def test_normalized_rows_feed_existing_analytics_without_glue():
    payload = {"o_curinstrument": [
        {"PRODUCTGROUPID": "cu", "DELIVERYMONTH": "2609", "OPENPRICE": 80000, "HIGHESTPRICE": 81000, "LOWESTPRICE": 79000, "CLOSEPRICE": 80500, "SETTLEMENTPRICE": 80300, "PRESETTLEMENTPRICE": 79800, "VOLUME": 123, "OPENINTEREST": 789, "TURNOVER": 456},
        {"PRODUCTGROUPID": "cu", "DELIVERYMONTH": "2610", "OPENPRICE": 80100, "HIGHESTPRICE": 81100, "LOWESTPRICE": 79100, "CLOSEPRICE": 80600, "SETTLEMENTPRICE": 80400, "PRESETTLEMENTPRICE": 79900, "VOLUME": 100, "OPENINTEREST": 600, "TURNOVER": 400},
    ]}
    rows = parse_shfe_daily_payload(payload, "20260814", "u")
...
class _AllExchangeClient:
    def __init__(self):
        self.calls = []

    def get_json(self, url, **kwargs):
        self.calls.append(("GET_JSON", url, kwargs))
        product = "sc" if "ine.cn" in url else "cu"
        return {"o_curinstrument": [{
            "PRODUCTGROUPID": product, "DELIVERYMONTH": "2609", "OPENPRICE": 100, "HIGHESTPRICE": 110,
            "LOWESTPRICE": 90, "CLOSEPRICE": 105, "SETTLEMENTPRICE": 104, "PRESETTLEMENTPRICE": 99,
            "VOLUME": 10, "OPENINTEREST": 20, "TURNOVER": 30,
        }]}
    ...
    def post_json(self, url, **kwargs):
        ...
        if "dce.com.cn" in url:
            return {"data": [{
                "variety": "豆粕", "contractId": "m2609", "open": 100, "high": 110, "low": 90, "close": 105,
                "lastClear": 99, "clearPrice": 104, "volumn": 10, "openInterest": 20, "turnover": 30,
            }]}
        return {"data": [{
            "variety": "碳酸锂", "varietyOrder": "lc", "delivMonth": "2609", "open": 100, "high": 110, "low": 90, "close": 105,
            "lastClear": 99, "clearPrice": 104, "volumn": 10, "openInterest": 20, "turnover": 30,
        }]}
    ...
    def get_response(self, url, **kwargs):
        ...
        if "cffex.com.cn" in url:
            text = "合约代码,今开盘,最高价,最低价,成交量,成交金额,持仓量,持仓变化,今收盘,今结算,前结算\nIF2609,4000,4100,3950,100,12345,200,1,4050,4040,3990\n"
            buffer = io.BytesIO()
            with zipfile.ZipFile(buffer, "w") as archive:
                archive.writestr("20260814_1.csv", text.encode("gb2312"))
            return _BinaryResponse(buffer.getvalue())
        text = "品种月份|昨结算|今开盘|最高价|最低价|今收盘|今结算|涨跌1|涨跌2|成交量(手)|空盘量|增减量|成交额(万元)|交割结算价\nCF609|15000|15100|15200|14900|15050|15040|50|40|1234|5678|10|9999.5|\n"
        return _BinaryResponse(text.encode("utf-8"))


def test_dispatcher_routes_all_six_exchange_transports():
    client = _AllExchangeClient()
    for exchange in ("SHFE", "INE", "DCE", "CZCE", "CFFEX", "GFEX"):
        rows = fetch_cn_futures_daily(exchange, "20260814", client=client)
        assert rows and rows[0]["exchange"] == exchange
        assert rows[0]["delivery_month"] in {"2609", "609"}
    methods = [call[0] for call in client.calls]
    assert methods.count("GET_JSON") == 2
    assert methods.count("POST_JSON") == 2
    assert methods.count("GET_RESPONSE") == 2
